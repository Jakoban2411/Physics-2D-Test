...
@onready var box = PhysicsServer2D.rectangle_shape_create()
...
func spawn_cubes() -> void:
	...
	var cube : InnerCube = InnerCube.new(PhysicsServer2D.body_create(), mm.visible_instance_count)
	PhysicsServer2D.body_set_mode(cube, PhysicsServer2D.BODY_MODE_RIGID)
	PhysicsServer2D.body_add_shape(cube.rID,box)
	PhysicsServer2D.body_set_space(cube.rID,get_world_2d().space)

	mm.set_instance_transform(cube.cID,spawn_transform)
	PhysicsServer2D.body_set_state(cube.rID,PhysicsServer2D.BODY_STATE_TRANSFORM,spawn_transform)
	PhysicsServer2D.body_apply_central_impulse(cube.rID,Vector2(randf_range(-5,5),-45))
