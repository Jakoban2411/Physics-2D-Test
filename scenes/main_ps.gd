...
func _physics_process(_delta) -> void:
	...
	for cube in cube_array:
		mm.set_instance_transform(cube.cID,PhysicsServer2D.body_get_direct_state(cube.rID).transform)
...
func spawn_cubes() -> void:
	...
	PhysicsServer2D.shape_set_data(box,Vector2(0.5,0.5))
	
	for i in range(num_cubes):
		# create collision shapes in Physics Server
		var cube = InnerCube.new(PhysicsServer2D.body_create(),i)
		PhysicsServer2D.body_set_mode(cube, PhysicsServer2D.BODY_MODE_RIGID)
		cube_array.append(cube)
		var transform = Transform2D()
		PhysicsServer2D.body_add_shape(cube_array[i].rID,box)
		PhysicsServer2D.body_set_param(cube_array[i].rID,PhysicsServer2D.BODY_PARAM_MASS, 1.0)
		PhysicsServer2D.body_set_param(cube_array[i].rID,PhysicsServer2D.BODY_PARAM_GRAVITY_SCALE, 1.0)
		PhysicsServer2D.body_set_space(cube_array[i].rID,get_world_2d().space)
		PhysicsServer2D.body_set_state(cube_array[i].rID,PhysicsServer2D.BODY_STATE_CAN_SLEEP,true)
		PhysicsServer2D.body_set_state(cube_array[i].rID,PhysicsServer2D.BODY_STATE_SLEEPING,true)
		...
		if i%4 == 0:
			transform = transform.translated(Vector2(a+1,level + 0.5))
			a+=1
		elif i%4 == 1:
			transform = transform.translated(Vector2(-side_length / 2.0 + side_length + 1.0, level + 0.5))
			b+=1
		elif i%4 == 2:
			transform = transform.translated(Vector2(c + 1.0, level + 0.5))
			c+=1
		else:
			transform = transform.translated(Vector2(-side_length / 2.0, level + 0.5))
			d+=1
		mm.set_instance_transform(cube_array[i].cID,transform)
		PhysicsServer2D.body_set_state(cube_array[i].rID,PhysicsServer2D.BODY_STATE_TRANSFORM,transform)
...
func deleteCubes() -> void:
	for cube in cube_array:
		PhysicsServer2D.free_rid(cube.rID)
	cube_array.clear()
...
func _on_Timer_timeout() -> void:
	...
	var sleep_count = 0
	for cube in cube_array:
		if PhysicsServer2D.body_get_direct_state(cube.rid).sleeping:
