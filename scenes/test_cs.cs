...
public partial class test_cs : Node2D
{
	...
	private Transform2D spawnTransform;
	...
	public override void _Ready()
	{
		...
		mm = GetNode<MultiMeshInstance2D>("MultiMeshInstance2D").Multimesh;
		...
		boxShape = PhysicsServer2D.ShapeCreate(PhysicsServer2D.ShapeType.Box);        
		PhysicsServer2D.ShapeSetData(boxShape,new Vector2(0.5f,0.5f));
		spawnTransform = Transform2D.Translated(new Vector2(0f, 35f)); // spawn point
		//setupObjectPool();
	}
	...
	private void setupObjectPool() 
	{
		...
		{
			cubeList.Add(new InnerCube(PhysicsServer2D.BodyCreate(PhysicsServer2D.BodyMode.Rigid, true), i)); // sleeping physics body            
		}
	}
	...
	public override void _PhysicsProcess(float delta)
	{
		...
		foreach (InnerCube c in cubeList) {
			mm.SetInstanceTransform(c.cID, PhysicsServer2D.BodyGetDirectState(c.rID).Transform2D);
		}
	}
	...
	private void spawnCubes() 
	{
		...
		InnerCube cube = new InnerCube(PhysicsServer2D.BodyCreate(PhysicsServer2D.BodyMode.Rigid, true),mm.VisibleInstanceCount++);
		// add collision shape to body + properties
		PhysicsServer2D.BodyAddShape(cube.rID,boxShape,Transform2D.Identity);
		// add to default world space
		PhysicsServer2D.BodySetSpace(cube.rID,this.GetWorld2d().Space);

		// set transform (spawn loc) of mesh
		mm.SetInstanceTransform(cube.cID, spawnTransform);
		PhysicsServer2D.BodySetState(cube.rID, PhysicsServer2D.BodyState.Transform2D, spawnTransform);
		PhysicsServer2D.BodyApplyCentralImpulse(cube.rID,new Vector2((float)r.NextDouble()*10-5,-45);
	...
	{        
		...
		foreach (InnerCube c in cubeList) 
		{
			if (PhysicsServer2D.BodyGetDirectState(c.rID).Sleeping) 
				sleepCount++;
		}
